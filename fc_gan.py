...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=00,n_classes=2):
    g_1_epoch = []
    g_2_epoch = []
    d_f1_epoch = []
    d_f2_epoch = []
...
##### Reading data using function from util
...
#train
(g_1_epoch,g_2_epoch,d_f1_epoch, d_f2_epoch, d_r1_epoch, d_r2_epoch) = train(g_model, d_model, gan_model, (X_train,Y_train), latent_dim,n_epochs=700, n_batch=10000,n_classes=2)

#real/fake loss plotting
import pandas as pd
...
#load best data model
Data = load_model('/Users/sahithis/Downloads/Deep learning research/cgan_model_700.h5')

# generate fake data 20000
X,y = generate_fake_samples(Data, 100,20000,2)# generate 10 example images
...
np.savetxt('X_train1.csv', X_train1.reshape(92696,804), delimiter=',')

y_train1 = np.r_[Y_train,X[1]]
y_train1.shape
np.savetxt('y_train1.csv', y_train1, delimiter=',')
...
filepa = "bestmodelnew_human.hdf5"
...
dnn_model.fit(X_train1, y_train1, batch_size= 190, epochs=20, shuffle=True, validation_data=(X_val,Y_val), callbacks=[checkpointer,earlystopper])
predictions = dnn_model.predict(X_test)
rounded = [round(x[0]) for x in predictions]
pred_train_prob = predictions
#Output from utils function metrics()
