...
def get_pred_num(fasta_name):
    num = 0
    for seq_record in SeqIO.parse(fasta_name,"fasta"):
        if(not(re.search('N',str(seq_record.seq)))):
            num+=1
    return num
...
def get_test_data(infile,infile2):
    rank = 0
    num = get_num(infile,infile2)
    seqmatrix = np.zeros((num,4,201))
    (seqmatrix, poslab, rank) = get_seq_matrix(infile,seqmatrix,rank)
    ...
    X = seqmatrix
    Y = np.array(poslab + neglab,dtype = int)
    return np.transpose(X,axes=(0,2,1)), Y

def get_pred_data(infile):
    rank = 0
    num = get_pred_num(infile)
    seqmatrix = np.zeros((num,4,201))
    (seqmatrix, gene, rank) = get_seq_matrix(infile,seqmatrix,rank)
    X = seqmatrix
    Y = np.array(gene,dtype = str)
    return np.transpose(X,axes=(0,2,1)), Y
...
def metrics(Y_test, rounded, pred_train_prob):
    ...
    print("Sepcificity: "+str(sepcificity)+"\n")
    print("Sensitivity: "+str(sensitivity)+"\n")
    print("MCC: "+str(mcc)+"\n")
    print("Fscore: "+str(f1)+"\n")
    print("AUC: "+str(aucvalue)+"\n")
    print("confusion matrix:"+str(confusion_matrix(Y_test, rounded))+"\n")
    print("Classification report:"+str(classification_report(Y_test, rounded))+"\n")
    return(accuracy)
